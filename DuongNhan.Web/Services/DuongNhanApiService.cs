    ...
    ISubscriptionApi subscriptions)
{
    ...
    public async Task<ApiResult> GoogleLoginAsync(string idToken, string? email = null, string? displayName = null, string? pictureUrl = null, CancellationToken ct = default)
    {
        var request = new DuongNhan.Shared.Dtos.Auth.GoogleLoginRequest(idToken, email, displayName, pictureUrl);
        var response = await auth.GoogleLoginAsync(request, ct);
        return ApiResult.From(response);
    }
    ...
    public async Task<ApiResult> UploadBytesAsync(byte[] bytes, string fileName = "camera-capture.jpg", string contentType = "image/jpeg", CancellationToken ct = default)
    {
        using var stream = new System.IO.MemoryStream(bytes);
        var response = await skin.UploadAsync(new StreamPart(stream, fileName, contentType), ct);
        return ApiResult.From(response);
    }
